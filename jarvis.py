...
class JarvisAssistant:
    def __init__(self) -> None:
        self.background_recognizer = sr.Recognizer()
        self.command_recognizer = sr.Recognizer()
        self.background_microphone = sr.Microphone()
        ...
        self.background_listen_lock = threading.Lock()
    ...
    def setup(self) -> None:
        ...
        with self.background_microphone as source:
            self.background_recognizer.adjust_for_ambient_noise(source, duration=1)
        logging.info("Jarvis готов к работе.")

    def speak(self, text: str) -> None:
        logging.info("TTS: %s", text)
        self.last_tts_at = time.monotonic()
        self.tts.say(text)
        self.tts.runAndWait()
    ...
    def _background_callback(self, recognizer: sr.Recognizer, audio: sr.AudioData) -> None:
        ...
        if time.monotonic() - self.last_tts_at < 2:
            logging.info("Игнор wake word: вероятно эхо собственного TTS.")
            return
    ...
    def _start_background_listening(self) -> None:
        with self.background_listen_lock:
            if self.listening_active:
                return
            self.stop_background = self.background_recognizer.listen_in_background(
                self.background_microphone,
    ...
    def _stop_background_listening(self) -> None:
        with self.background_listen_lock:
            if not self.listening_active:
                return
            if self.stop_background:
                self.stop_background(wait_for_stop=True)
                self.stop_background = None
            self.listening_active = False
            logging.info("Фоновое прослушивание временно остановлено.")
    ...
    def _listen_for_command(self, timeout: int = 5) -> str:
        ...
        with sr.Microphone() as source:
            try:
                audio = self.command_recognizer.listen(source, timeout=1, phrase_time_limit=timeout)
            except sr.WaitTimeoutError:
                return ""
        try:
            return self.command_recognizer.recognize_google(audio, language="ru-RU").lower().strip()
        except sr.UnknownValueError:
